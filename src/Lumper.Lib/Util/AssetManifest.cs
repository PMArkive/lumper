...
public static class AssetManifest
{
    ...
    private static Dictionary<string, List<Asset>> Load()
    {
        try
        {
            ...
            var end = reader.BaseStream.Length - 2;
            ...
            while (reader.BaseStream.Position <= end)
            {
                ...
                var vpk = decoder.GetString(buffers[0], 0, bufferLengths[0]);
                var path = decoder.GetString(buffers[1], 0, bufferLengths[1]);
                var hash = decoder.GetString(buffers[3], 0, bufferLengths[3]);

                var vpkFirstSlash = vpk.IndexOf('/');
                string origin;
                ...
                {
                    var pathFirstSlash = path.IndexOf('/');
